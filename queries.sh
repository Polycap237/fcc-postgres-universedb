...
echo "$($PSQL "SELECT DISTINCT teams.name FROM teams JOIN games ON (teams.team_id = games.winner_id OR teams.team_id = games.opponent_id) WHERE games.round = 'Eighth-Final' AND games.year = 2014 ORDER BY teams.name ASC")"
